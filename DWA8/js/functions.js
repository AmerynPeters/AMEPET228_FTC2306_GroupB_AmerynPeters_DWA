...
/* ---------------------------------------GLOBAL VARIABLES--------------------------------------- */
export const {
    home: { root: ROOT, main: HOME_PAGE, showMoreBtn: SHOW_MORE_BTN },
    summary: { overlay: BOOK_SUMMARY },
} = data;
...
export const descriptionOverlay = (event) => {
    ...
    books.forEach((book) => {
        ...
        <h3 class="overlay__title" data-list-title="">${book.title} (${new Date(
                    book.published
                ).getFullYear()})</h3>
...
const createBookButtons = (book) => {
    ...
    button.innerHTML =
        /* HTML markup for the book cards */
        `
...
export const appendBooks = (books) => {
    ...
    SHOW_MORE_BTN.innerHTML = `Show more <span class = "list__remaining">(${
        updateBooksLeft().booksLeft
    })</span>`;
...
export const searchBooks = (event) => {
    ...
    const searchText = getHtml("[data-search-title]")
        .value.toLowerCase()
        .trim();
...
export const changeTheme = (event) => {
    ...
    const themeOption = getHtml("[data-settings-theme]").querySelectorAll(
        "option"
    );
